...
export function generateQuotePDF({
  ...
  const documentMeta = createQuoteDocumentMeta({
    quoteDate,
    quoteStatus,
    quoteValidityDays: company.quoteValidityDays,
    savedQuoteNumber,
  });
  const { issuedDate, quoteNumber, validUntil } = documentMeta;
  ...
  doc.setFont("helvetica", "bold");
  doc.text(`Status: ${documentMeta.statusLabel}`, 150, 46);
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      ...
      2: { halign: "right" },
      // Keep the Qty/Hrs heading on one line in the exported A4 table.
